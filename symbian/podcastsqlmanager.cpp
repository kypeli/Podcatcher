    ...
    QObject(parent)
{
    ...
    databasePath = databasePath.append("podcatcher.sql");
    m_connection = QSqlDatabase::addDatabase("QSQLITE");
    m_connection.setDatabaseName(databasePath);
    ...
    qDebug() << "Tabled: " << m_connection.tables();
...
QList<PodcastEpisode *> PodcastSQLManager::episodesInDB(int channelId)
{
    ...
    while (q.next()) {
        PodcastEpisode *episode = PodcastEpisode::newEpisode();
